...
model {
  ...
  for (i in 1:length(whp)) {
    # elliptic
    # mu2[i] <- beta_whp[well_id[i]] * whp_c[i] + beta_whp2[well_id[i]] * whp_c[i]^2 + beta_date[well_id[i]] * date_numeric[i] + Intercept[well_id[i]]
    ...
    # exponential
    # mu[i] <- beta_whp[well_id[i]] * (Intercept[well_id[i]] - whp_c[i]^2) ^ beta_date[well_id[i]]
    # quadratic
  ...
  for (i in 1:length(whp_prod)) {
    # elliptic
    # mf_prod2[i] <- beta_whp[well_id_prod[i]] * whp_c_prod[i] + beta_whp2[well_id_prod[i]] * whp_c_prod[i]^2 + beta_date[well_id_prod[i]] * today_numeric + Intercept[well_id_prod[i]]
  ...
  for (i in well_ids) {
    # elliptic
    # mf_pred2[i] <- beta_whp[i] * whp_c_pred[i] + beta_whp2[i] * whp_c_pred[i]^2 + beta_date[i] * today_numeric + Intercept[i]
...
vars =  c(paste0('mf_fit[', 1:length(data$whp), ']'),
          ...
          'total_power',
          paste0('mu_', c('Intercept', 'beta_whp', 'beta_date', 'beta_whp2')))
n_chains = 2
burn_in = 250
n_steps = 10000
...
testout = coda.samples(testmodel, n.iter=1000, variable.names=c("beta", "alpha")) %>%
  as.matrix() %>%
  as.data.frame()
