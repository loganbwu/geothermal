...
g6 = ggplot(outframe %>% filter(variable=="sd") %>% filter(facility %in% c('wk124', 'wk242', 'wk263', 'wk270', 'wk271')), aes(x=value, fill=facility)) +
...
tb2 <- outframe %>% filter(variable=="beta_date") %>% select(facility, value) %>%
  ...
  mutate_if(is.numeric, round, 3) %>%
  inner_join(regression_df %>% mutate(well=factor(names(ids)[well_id])) %>% group_by(well) %>% summarise(n=n()))
tb2[tb2$well %in% c('wk124', 'wk242', 'wk263', 'wk270', 'wk271'),] %>% knitr::kable()

tb6[tb6$well %in% c('wk124', 'wk242', 'wk263', 'wk270', 'wk271'),] %>% knitr::kable()
```
# Hyper-parameters
```{r hyperparameters}
