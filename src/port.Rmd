...
censor = function(x, type) {
  if (type=="well") {
    ...
    return(paste0("fp", substr(md5(x), 1, 2)))
  }
}
```
...
regression_df = read_excel(regdatapath) %>% mutate(source="Wellbore Tests")
dry_df = PI %>%
  filter(str_detect(facility, "wk")) %>%
  select(facility, date, variable, value) %>%
  filter(value>1e-2) %>%
...
Censor well and flash plant names using the MD5 hash algorithm.
...
ggplot(prod, aes(x=whp)) +
  ...
  labs(title=paste("Linear Regression with Time on PI Data", production_curve_well), x="Well-head pressure (bar)", y="Mass flow (T/h)", color="Well", shape="Data source") +
...
tsplotdata = dry_df %>%
  filter(well_id %in% ids[special_wells], well_id %in% ids[ar_well_ids]) %>%
  mutate(datetime = factor(as.Date(date)),
         facility = well)
...
ggplot(flow.df, aes(x=value)) +
  geom_density(aes(y=..scaled.., fill=variable, color=variable), alpha=0.5, show.legend=F) +
  geom_vline(data=comparison, aes(xintercept=value)) +
  geom_label(data=ps, aes(x=-Inf, y=Inf, hjust=0, vjust=1, label=paste0("p(>x)=", p.greater), family="Times New Roman"), alpha=0.5) +
  facet_grid(facility~variable, scales="free") +
  theme(axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) +
  labs(x="Value", y="Scaled density", title="Comparison Between Predicted FP Flows and Sample Data") +
  ggsave('../media/verification.png', width=24.7, height=24, units='cm')
```
...
geweke.out = geweke.diag(out[,1:100])
...
ggplot(geweke.df, aes(x=Index, y=z)) +
  ...
  labs(title=paste("Geweke z-score", round(proportion_out, 2), "points lie outside the 95% confidence interval.")) +
