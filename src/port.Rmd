...
Censor well and flash plant names using the MD5 hash algorithm. Comment this block to create plots and outputs with real facility names.
...
well_summaries = outframe %>%
  ...
  summarise(mean = mean(value),
            sd = sd(value),
            n_test = nrow(regression_df %>% filter(well==facility, source=="Well Tests")),
            n_pi = nrow(regression_df %>% filter(well==facility, source=="PI Database")),
...
ggplot(well_summaries, aes(x=1, y=sd)) +
  geom_boxplot(fill='steelblue') +
  facet_grid(.~ use.pi + use.test) +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +
  labs(title="Differences in Production Error by Data Source", x="Production curve data source", y="Standard deviation") +
...
```{r traceplots}
trace1 <- outframe %>%
  filter(variable=='mf_pred' & (facility==censor('wk256', "well") | facility=='wk256'))
trace2 <- outframe %>%
