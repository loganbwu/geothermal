...
g1 = ggplot(outframe %>% 
  ...
  ggsave('../media/mf_wells.png', width=24.7, height=8, units='cm')
...
tb2 <- outframe %>% filter(variable=="beta_date") %>% select(facility, value) %>%
  ...
  summarise(Mean = mean(value), 
            `Lower 2.5%` = quantile(value, 0.025), 
            `Upper 97.5%` = quantile(value, 0.975)) %>%
  left_join(regression_df %>% mutate(well=factor(names(ids)[well_id])) %>% group_by(well) %>% summarise(n=n()), by="well")
...
ggplot(flow.df %>% filter(facility %ni% incomplete.fps), aes(x=value)) +
  ...
  ggsave('../media/verification.png', width=24.7, height=20, units='cm')
```
