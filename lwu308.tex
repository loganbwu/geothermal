...
\todo{Feels like I'm describing what I've done, not summarising. Let me know.}We implement a Bayesian network model that performs a statistical simulation of the Wairakei surface network, simultaneously incorporating data from both sources and over a long period of time to estimate the network parameters.\\
...
Most of the parameters pass Geweke's test with a z-score (a Normal approximation of the T-statistic) less than 1.96 for a 95\% confidence interval. \todo{Increase sample size for final run}However, the changes in the power output traces were significant, so a short burn-in of 200 iterations was introduced.
...
\lstinputlisting[language=R]{src/model.txt}
